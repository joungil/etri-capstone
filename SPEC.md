...
- `web_api.py`는 루프백에만 바인딩하며, 바인딩 주소를 설정으로 노출하지 않는다. 이 API에는 인증이 없어 루프백 바인딩이 유일한 접근 제어이므로, 주소를 바꿀 수 있게 두면 그 제어가 설정 하나로 사라진다. 다른 기기에서 읽어야 한다면 인증과 접근 제어를 먼저 갖추는 것이 선행 조건이다.
