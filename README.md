...
상세한 요구사항과 계약은 `SPEC.md`에, 웹 뷰어의 시각 디자인 결정은 `DESIGN.md`에 정리되어 있다.
...
`web_api.py`가 바인딩할 주소는 환경변수로 두지 않았다. 이 API에는 인증이 없어서 루프백에만 붙는 것이 유일한 접근 제어이고, 주소를 바꿀 이유가 곧 외부 노출이기 때문이다. 다른 기기에서 리포트를 읽어야 한다면 주소를 바꾸기 전에 인증과 접근 제어를 먼저 갖춰야 한다.
